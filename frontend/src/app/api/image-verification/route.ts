...
async function recordImageToBlockchain({
  ...
  try {
    ...
    let requestBody: any = {
      ...
      farmerId,
      stageId,
      stageName: stageData.name, // ✅ Required by Blockchain Bridge
      imageUrl: imageVerification.imageUrl,
      verificationId: imageVerification.id,
      isFirstImage
    };

    // If first image, fetch farmer and batch details
    if (isFirstImage) {
      ...
      const { data: batchData, error: batchError } = await supabase
        .from('batches')
        .select('name, category, area, sowingDate, description, location')
        .eq('id', batchId)
        .single();
      ...
      requestBody.batchDetails = {
        cropName: batchData.name,
        cropType: batchData.category,
        variety: batchData.description || '',
        quantity: batchData.area,
        unit: 'hectares',
        expectedHarvestDate: batchData.sowingDate || '',
        farmLocation: batchData.location ? JSON.stringify(batchData.location) : ''
      };
    }

    // Call blockchain API
    const blockchainUrl = process.env.NODE_ENV === 'production'
      ? 'http://localhost:8080/record-image'
      : 'http://localhost:8080/record-image';

    console.log(`🔗 Calling blockchain API: ${blockchainUrl}`);
    ...
    const response = await fetch(blockchainUrl, {
      ...
      body: JSON.stringify({ action: 'recordImage', data: requestBody })
    });
    ...
    console.log('🔍 Checking blockchain response for QR...');
    console.log('   qrGenerated:', blockchainResult.qrGenerated);
    console.log('   has qrCode:', !!blockchainResult.qrCode);
