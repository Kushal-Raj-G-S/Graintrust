...
export async function generateCertificate(batch: BatchData): Promise<void> {
  try {
    ...
    doc.text('GRAINTRUST', pageWidth / 2, 18, { align: 'center' });
    ...
    doc.text('GrainTrust Official', 57.5, footerY + 28, { align: 'center' });
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'normal');
    doc.text('GrainTrust - Blockchain-Powered Agricultural Transparency', pageWidth / 2, pageHeight - 7, { align: 'center' });
